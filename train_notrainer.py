...
from customized_classifier.model.distilbert import CustomizedDistilbertClassifier
from customized_classifier.data.data_collator import customized_data_collator
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='double',
        help="single: predict single label. double: predict two labels at the same time",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max number of training samples. Useful for quick checks on training code.",
    )
    parser.add_argument(
        "--max_eval_samples",
        type=int,
        default=None,
        help="Max number of eval samples. Useful for quick checks on training code.",
    )
...
# https://huggingface.co/docs/datasets/loading_datasets.html.
    ...
    if args.task_name == 'double':
        config.num_labels_2 = num_labels_2
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_fast=not args.use_slow_tokenizer)

    if args.task_name == 'double':
        model = CustomizedDistilbertClassifier.from_pretrained(
    ...
    if args.task_name == 'double':
        metric_acc_2 = load_metric("accuracy")
        metric_f1_2 = load_metric("f1")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            if args.task_name == 'double':
                predictions_2 = outputs.logits_2.argmax(dim=-1)
                metric_acc_2.add_batch(
                    predictions=accelerator.gather(predictions_2),
                    references=accelerator.gather(batch["labels_2"]),
                )
                metric_f1_2.add_batch(
                    predictions=accelerator.gather(predictions_2),
                    references=accelerator.gather(batch["labels_2"]),
                )

        eval_metric_acc = metric_acc.compute()
        eval_metric_f1 = metric_f1.compute(average='weighted')
        if args.task_name == 'double':
            eval_metric_acc_2 = metric_acc_2.compute()
            eval_metric_f1_2 = metric_f1_2.compute(average='weighted')
            logger.info(f"epoch {epoch}: Label 1 > {eval_metric_acc}, {eval_metric_f1}. Label 2 > {eval_metric_acc_2}, {eval_metric_f1_2}")
        else:
            logger.info(f"epoch {epoch}: {eval_metric_acc}, {eval_metric_f1}")
